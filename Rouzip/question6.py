...
def statistics_file(root=root):
    parser = argparse.ArgumentParser(
        prog='have fun', description='个人小脚本，统计文件数量')
    parser.add_argument('-p', '--path', nargs='?',
                        help='统计文件夹位置', default=os.getcwd())
    parser.add_argument('-t', '--type', nargs='?', help='统计文件种类',
                        choices=['python', 'java'])
    ...
    if len(sys.argv[1:]) == 0:
        ...
        for _, dirs, files in os.walk(kargs.path):
            ...
            dir_num += len(dirs)
        # cprint('文件个数为：' + str(file_num), 'red', attrs=['bold'])
        # cprint('文件夹个数为：' + str(dir_num), 'red', attrs=['bold'])
    # 指定了文件夹地址，没有选择解析文件
